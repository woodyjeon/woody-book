...
- 책 목록(페이지네이션·카테고리/장르 필터·정렬) / 상세(평균 별점·위시 카운트)
- 리뷰 작성/조회/삭제
- 댓글 작성/조회/삭제

다음 단계: 위시리스트 API, 검색, 추천, 마이페이지
...
PostgreSQL 16-alpine 컨테이너가 실행됩니다.
...
python -m app.seed   # 샘플 책 데이터 + 테스트 계정(test_account / test!1234) 추가
